...
public class FocLinkN2N extends FocLink {
  ...
  public boolean loadDB(FocList focList) {
    ...
    if (loadAll) {
      ...
      /*
      SQLSelect select = new SQLSelect(linkTableDesc, filter);
      select.execute();
      FocList loadedFocList = select.getFocList();
      */

      /*
      FocConstructor constr = new FocConstructor(getSlaveDesc(), null ,null);
      FocObject slaveTemporaryObject = constr.newItem();
      slaveTemporaryObject.setDbResident(false);
      FProperty slaveTemporaryProperty = slaveTemporaryObject.getIdentifierProperty();
      */
