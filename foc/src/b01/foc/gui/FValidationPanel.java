...
public class FValidationPanel extends FPanel implements FIFooterPanel {
	...
	private static final long serialVersionUID = 9088597083354259496L;
  ...
  FValidationPanel(boolean normalButtonsPositioning, String frameName) {
    super();
    setName(frameName+".FVALIDATIONPANEL");
  ...
  public void doValidate(){
    ...
    if(validListener == null || validListener.proceedValidation(this)){
      boolean isValidated = accessConsole.validate(true);
      if(isValidated){
    ...
    if (accessConsole != null) {
      ...
      while(iter != null && iter.hasNext()){
        ...
        if (currSubject != null) {
          ...
          if (currSubject.getClass() == FocList.class) {
  ...
  private void setButtonsAccordingToFlags(boolean normalButtonsPositioning) {
    ...
    int validationButtonX = 0;
    int cancelationButtonX = 1;
    
    if(!normalButtonsPositioning){
      validationButtonX = 1;
      cancelationButtonX = 0;
    }
    boolean validButtonVisible = true;// The validation is allways visible
    ...
    if (validButtonVisible) {// If visible we need to set it
      ...
      if (validationButton == null) {
        ...
        if(ConfigInfo.isUnitDevMode()){
          validationButton.setToolTipText(validationButton.getName());
        }else{
          validationButton.setToolTipText(validationToolTipText);  
        }
      }
    ...
    boolean cancelButtonVisible = validationType != VALIDATION_OK;// The
                                                                  // validation
                                                                  // is allways
                                                                  // visible
    ...
    if (cancelButtonVisible) {// If visible we need to set it
      ...
      if (cancelationButton == null) {
        ...
        cancelationButton.setName(getName()+"."+BUTTON_CANCEL);
        ...
        if(ConfigInfo.isUnitDevMode()){
          ...
          cancelationButton.setToolTipText(cancelationButton.getName());
        }else{
          cancelationButton.setToolTipText(cancelationTTT);  
        }
      }
    }
  ...
  public boolean isAskForConfirmationForExit() {
    //BELIAS
    //return askForConfirmationForExit;
