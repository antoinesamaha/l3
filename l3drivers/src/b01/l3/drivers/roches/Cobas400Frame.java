 ...
 * @author 01Barmaja
 */
public class Cobas400Frame extends L3Frame {

	private static HashMap<String, String> error = null;
	private static final String FRAME_START = String.valueOf(Cobas400IntegraDriver.SOH)+ String.valueOf(Cobas400IntegraDriver.LF);
	private static final String FRAME_END = String.valueOf(Cobas400IntegraDriver.EOT)+ String.valueOf(Cobas400IntegraDriver.LF);
	...
	public Cobas400Frame(Instrument instrument, int instrumentCode, String instrumentName,	int blockCode, int sequence) {
		super(instrument);
	...
	public Cobas400Frame(Instrument instrument) {
		super(instrument);
  ...
  private void fillError(){
	...
	public void createDataWithFrame() throws Exception {
		...
		append2DataWithFrame(Cobas400IntegraDriver.SPACE);
		...
		append2DataWithFrame(Cobas400IntegraDriver.LF);
		// ------------

		append2DataWithFrame(Cobas400IntegraDriver.STX);
		append2DataWithFrame(Cobas400IntegraDriver.LF);

		append2DataWithFrame(data);

		append2DataWithFrame(Cobas400IntegraDriver.ETX);
		append2DataWithFrame(Cobas400IntegraDriver.LF);

		if (blockCode != Cobas400IntegraDriver.BLOCK_CODE_SYNCHRONIZATION) {
	...
	public void extractDataFromFrame() throws Exception {
		...
		StringTokenizer stt = new StringTokenizer(getDataWithFrame().toString(), String.valueOf(Cobas400IntegraDriver.LF));
		...
		while (stt.hasMoreElements()) {
			...
			if(line.compareTo(String.valueOf(Cobas400IntegraDriver.ETX)) == 0){
				break;
			}
			if(lineIdx == 1) {
        blockCode = Integer.valueOf(line.substring(20,22)).intValue();
				
				if (blockCode != Cobas400IntegraDriver.BLOCK_CODE_SYNCHRONIZATION && !checkCheckSum()) {
	...
	private void append2DataWithLeftBlank(StringBuffer buffer, String inc, int fixedLength) {
		...
		for (int i = 0; i < fixedLength - inc.length(); i++) {
			buffer.append(Cobas400IntegraDriver.SPACE);
		}
	}
