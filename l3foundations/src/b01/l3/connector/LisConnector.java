        ...
        if (!stopDbPooling){
          if(isConnected()){
        	Globals.logString("P6-Prepare Filter and Set Active");
          	...
          	java.sql.Date computerDate = new java.sql.Date(System.currentTimeMillis() - 2 * 30 * Globals.DAY_TIME);
          	filter.setEntryDate(DateCondition.OPERATOR_GREATER_THAN, computerDate, computerDate);
          	filter.setBlockedEquals(false);//20150917
